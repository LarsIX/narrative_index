from pathlib import Path
from typing import Iterable, List, Optional, Tuple

import numpy as np
import pandas as pd
from statsmodels.tsa.stattools import adfuller, kpss
from arch.unitroot import PhillipsPerron


# --------------------------------------------------------------------- #
# Paths & helpers
# --------------------------------------------------------------------- #

def _project_paths() -> Tuple[Path, Path]:
    """
    Returns (variables_path, tables_path) and ensures they exist.
    """
    root = Path(__file__).resolve().parents[2]
    var_path = root / "data" / "processed" / "variables"
    table_path = root / "reports" / "tables"
    var_path.mkdir(parents=True, exist_ok=True)
    table_path.mkdir(parents=True, exist_ok=True)
    return var_path, table_path


def _clean_series(x: pd.Series) -> pd.Series:
    """
    Ensure numeric, finite, drop NaNs.
    """
    return (
        pd.to_numeric(x, errors="coerce")
        .replace([np.inf, -np.inf], np.nan)
        .dropna()
    )


def _run_adf_pp_kpss(
    series: pd.Series,
    kpss_regression: str = "c",
    kpss_nlags: str | int = "auto",
) -> Tuple[float, float, float, float, float, float]:
    """
    Compute ADF, PP, KPSS. Returns:
    (adf_stat, adf_p, pp_stat, pp_p, kpss_stat, kpss_p)
    """
    # ADF (autolag AIC)
    adf_stat, adf_p, *_ = adfuller(series, autolag="AIC")

    # PP (default Bartlett kernel)
    pp = PhillipsPerron(series)
    pp_stat = float(pp.stat)
    pp_p = float(pp.pvalue)

    # KPSS (null: stationarity). If it fails, surface as NaN.
    try:
        kpss_stat, kpss_p, *_ = kpss(
            series, regression=kpss_regression, nlags=kpss_nlags
        )
    except Exception:
        kpss_stat, kpss_p = np.nan, np.nan

    return adf_stat, adf_p, pp_stat, pp_p, kpss_stat, kpss_p


def _subset_periods(df: pd.DataFrame) -> dict:
    """
    Common period splits used across tests.
    """
    return {
        "2023": df[df["date"] < "2024-01-01"],
        "2024": df[(df["date"] >= "2024-01-01") & (df["date"] < "2025-01-01")],
        "2025": df[df["date"] >= "2025-01-01"],
        "2023_24": df[df["date"] < "2025-01-01"],
        "2024_25": df[df["date"] >= "2024-01-01"],
        "2023_25": df,  # full
    }


def _agree_stationary(adf_p: float, pp_p: float, kpss_p: float, alpha: float) -> bool:
    """
    Stationarity “agreement” rule:
    - ADF rejects unit root (adf_p < alpha)
    - PP  rejects unit root (pp_p  < alpha)
    - KPSS does NOT reject stationarity (kpss_p > alpha)

    If any is NaN, return False.
    """
    if any(pd.isna([adf_p, pp_p, kpss_p])):
        return False
    return (adf_p < alpha) and (pp_p < alpha) and (kpss_p > alpha)


# --------------------------------------------------------------------- #
# AINI variants: ADF, PP, KPSS
# --------------------------------------------------------------------- #

def test_stationarity_aini_variants(
    aini_data: pd.DataFrame,
    variants: Optional[str] = None,
    window: Optional[int] = None,
    aini_cols: Optional[Iterable[str]] = None,
    alpha: float = 0.05,
    min_obs: int = 20,
    kpss_regression: str = "c",
    kpss_nlags: str | int = "auto",
) -> pd.DataFrame:
    """
    Run ADF, PP, KPSS on selected AINI columns over multiple time windows.

    Parameters
    ----------
    aini_data : DataFrame with at least 'date' and the AINI columns.
    variants  : str label for the dataset (used in filenames).
    window    : None, 0, 1, 2 – only used in filenames/metadata.
    aini_cols : iterable of column names to test. Defaults to common AINI fields.
    alpha     : significance level for agreement.
    min_obs   : minimum observations to attempt a unit-root test.
    kpss_regression : 'c' (level) or 'ct' (trend) for KPSS.
    kpss_nlags : 'auto' or integer.

    Returns
    -------
    DataFrame of results; also saved to CSV/HTML.
    """
    var_path, table_path = _project_paths()

    if aini_cols is None:
        aini_cols = ["normalized_AINI", "EMA_02", "EMA_08"]

    df = aini_data.copy()
    df["date"] = pd.to_datetime(df["date"])
    subsets = _subset_periods(df)

    rows: List[dict] = []
    for period, dsub in subsets.items():
        for col in aini_cols:
            try:
                s = _clean_series(dsub[col])
                n = int(s.shape[0])
                if n < min_obs:
                    rows.append({
                        "Period": period,
                        "AINI_variant": col,
                        "Context_window": window,
                        "N": n,
                        "ADF_stat": np.nan, "ADF_p": np.nan,
                        "PP_stat": np.nan, "PP_p": np.nan,
                        "KPSS_stat": np.nan, "KPSS_p": np.nan,
                        "agree_stationarity": False,
                        "error": f"too_few_obs(<{min_obs})"
                    })
                    continue

                adf_stat, adf_p, pp_stat, pp_p, kpss_stat, kpss_p = _run_adf_pp_kpss(
                    s, kpss_regression=kpss_regression, kpss_nlags=kpss_nlags
                )
                rows.append({
                    "Period": period,
                    "AINI_variant": col,
                    "Context_window": window,
                    "N": n,
                    "ADF_stat": adf_stat, "ADF_p": adf_p,
                    "PP_stat": pp_stat, "PP_p": pp_p,
                    "KPSS_stat": kpss_stat, "KPSS_p": kpss_p,
                    "agree_stationarity": _agree_stationary(adf_p, pp_p, kpss_p, alpha),
                    "error": ""
                })

            except Exception as e:
                rows.append({
                    "Period": period,
                    "AINI_variant": col,
                    "Context_window": window,
                    "N": int(dsub.shape[0]),
                    "ADF_stat": np.nan, "ADF_p": np.nan,
                    "PP_stat": np.nan, "PP_p": np.nan,
                    "KPSS_stat": np.nan, "KPSS_p": np.nan,
                    "agree_stationarity": False,
                    "error": str(e)
                })

    out = pd.DataFrame(rows).sort_values(["Period", "AINI_variant"]).reset_index(drop=True)

    # Filenames 
    suffix = f"window{window}_" if (window is not None) else ""
    label = variants or "aini"
    csv_file = var_path / f"stationarity_tests_{suffix}{label}.csv"
    html_file = table_path / f"stationarity_tests_{suffix}{label}.html"

    out.to_csv(csv_file, index=False)
    out.to_html(html_file, index=False)
    print(f"Saved AINI stationarity results -> CSV: {csv_file} | HTML: {html_file}")
    return out


# --------------------------------------------------------------------- #
# Financial variables: ADF, PP, KPSS
# --------------------------------------------------------------------- #

def test_stationarity_fin_variables(
    fin_data: pd.DataFrame,
    fin_vars: Optional[Iterable[str]] = None,
    alpha: float = 0.05,
    min_obs: int = 20,
    date_col: str = "Date",
    ticker_col: str = "Ticker",
    label: str = "fin_var",
    kpss_regression: str = "c",
    kpss_nlags: str | int = "auto",
) -> pd.DataFrame:
    """
    Run ADF, PP, KPSS on financial variables, grouped by ticker and time window.

    Parameters
    ----------
    fin_data  : DataFrame with date_col, ticker_col, and variables in fin_vars.
    fin_vars  : iterable of variables (e.g., ["Adj Close", "LogReturn"]).
    alpha     : significance level for agreement.
    min_obs   : minimum observations.
    date_col  : name of the date column in fin_data.
    ticker_col: name of the ticker column.
    label     : filename label.
    kpss_regression : 'c' or 'ct' for KPSS.
    kpss_nlags : 'auto' or integer.

    Returns
    -------
    DataFrame; also saved to CSV/HTML.
    """
    var_path, table_path = _project_paths()

    if fin_vars is None:
        fin_vars = ["Adj Close", "LogReturn"]

    df = fin_data.copy()
    df["date"] = pd.to_datetime(df[date_col])
    subsets = _subset_periods(df)

    rows: List[dict] = []
    for period, dsub in subsets.items():
        for ticker, g in dsub.groupby(ticker_col, dropna=True):
            for var in fin_vars:
                try:
                    s = _clean_series(g[var])
                    n = int(s.shape[0])
                    if n < min_obs:
                        rows.append({
                            "Period": period, "Ticker": ticker, "Variable": var, "N": n,
                            "ADF_stat": np.nan, "ADF_p": np.nan,
                            "PP_stat": np.nan, "PP_p": np.nan,
                            "KPSS_stat": np.nan, "KPSS_p": np.nan,
                            "agree_stationarity": False,
                            "error": f"too_few_obs(<{min_obs})"
                        })
                        continue

                    adf_stat, adf_p, pp_stat, pp_p, kpss_stat, kpss_p = _run_adf_pp_kpss(
                        s, kpss_regression=kpss_regression, kpss_nlags=kpss_nlags
                    )
                    rows.append({
                        "Period": period, "Ticker": ticker, "Variable": var, "N": n,
                        "ADF_stat": adf_stat, "ADF_p": adf_p,
                        "PP_stat": pp_stat, "PP_p": pp_p,
                        "KPSS_stat": kpss_stat, "KPSS_p": kpss_p,
                        "agree_stationarity": _agree_stationary(adf_p, pp_p, kpss_p, alpha),
                        "error": ""
                    })
                except Exception as e:
                    rows.append({
                        "Period": period, "Ticker": ticker, "Variable": var, "N": int(g.shape[0]),
                        "ADF_stat": np.nan, "ADF_p": np.nan,
                        "PP_stat": np.nan, "PP_p": np.nan,
                        "KPSS_stat": np.nan, "KPSS_p": np.nan,
                        "agree_stationarity": False,
                        "error": str(e)
                    })

    out = (
        pd.DataFrame(rows)
        .sort_values(["Period", "Ticker", "Variable"])
        .reset_index(drop=True)
    )

    csv_file = var_path / f"stationarity_tests_{label}.csv"
    html_file = table_path / f"stationarity_tests_{label}.html"
    out.to_csv(csv_file, index=False)
    out.to_html(html_file, index=False)
    print(f"Saved FIN stationarity results -> CSV: {csv_file} | HTML: {html_file}")
    return out


# --------------------------------------------------------------------- #
# VIX (log_growth_closed): ADF, PP, KPSS
# --------------------------------------------------------------------- #

def test_stationarity_vix_zscores(
    vix_path: Optional[Path] = None,
    date_col: str = "date",
    value_col: str = "log_growth_closed",
    alpha: float = 0.05,
    min_obs: int = 20,
    kpss_regression: str = "c",
    kpss_nlags: str | int = "auto",
    label: str = "vix_zscores",
) -> pd.DataFrame:
    """
    Load VIX z-scores CSV and run ADF, PP, KPSS on 'log_growth_closed' across the same period splits.

    By default expects file at:
        data/processed/variables/z_scores_VIX.csv

    Parameters
    ----------
    vix_path : optional Path to CSV; if None, uses project var_path.
    date_col : name of date column in the CSV.
    value_col: name of the VIX z-score column to test ('log_growth_closed').
    alpha    : significance level for agreement.
    min_obs  : minimum observations per period.
    kpss_regression : 'c' or 'ct' for KPSS.
    kpss_nlags : 'auto' or integer.
    label    : filename label for outputs.

    Returns
    -------
    DataFrame; also saved to CSV/HTML.
    """
    var_path, table_path = _project_paths()
    if vix_path is None:
        vix_path = var_path / "z_scores_VIX.csv"

    df = pd.read_csv(vix_path)
    if date_col not in df.columns:
        raise ValueError(f"Date column '{date_col}' not found in {vix_path}")
    if value_col not in df.columns:
        raise ValueError(f"Value column '{value_col}' not found in {vix_path}")

    df = df.copy()
    df["date"] = pd.to_datetime(df[date_col])
    df = df.sort_values("date").reset_index(drop=True)
    subsets = _subset_periods(df)

    rows: List[dict] = []
    for period, dsub in subsets.items():
        try:
            s = _clean_series(dsub[value_col])
            n = int(s.shape[0])
            if n < min_obs:
                rows.append({
                    "Period": period, "Variable": value_col, "N": n,
                    "ADF_stat": np.nan, "ADF_p": np.nan,
                    "PP_stat": np.nan, "PP_p": np.nan,
                    "KPSS_stat": np.nan, "KPSS_p": np.nan,
                    "agree_stationarity": False,
                    "error": f"too_few_obs(<{min_obs})"
                })
                continue

            adf_stat, adf_p, pp_stat, pp_p, kpss_stat, kpss_p = _run_adf_pp_kpss(
                s, kpss_regression=kpss_regression, kpss_nlags=kpss_nlags
            )
            rows.append({
                "Period": period, "Variable": value_col, "N": n,
                "ADF_stat": adf_stat, "ADF_p": adf_p,
                "PP_stat": pp_stat, "PP_p": pp_p,
                "KPSS_stat": kpss_stat, "KPSS_p": kpss_p,
                "agree_stationarity": _agree_stationary(adf_p, pp_p, kpss_p, alpha),
                "error": ""
            })
        except Exception as e:
            rows.append({
                "Period": period, "Variable": value_col, "N": int(dsub.shape[0]),
                "ADF_stat": np.nan, "ADF_p": np.nan,
                "PP_stat": np.nan, "PP_p": np.nan,
                "KPSS_stat": np.nan, "KPSS_p": np.nan,
                "agree_stationarity": False,
                "error": str(e)
            })

    out = pd.DataFrame(rows).sort_values(["Period"]).reset_index(drop=True)

    csv_file = var_path / f"stationarity_tests_{label}.csv"
    html_file = table_path / f"stationarity_tests_{label}.html"
    out.to_csv(csv_file, index=False)
    out.to_html(html_file, index=False)
    print(f"Saved VIX stationarity results -> CSV: {csv_file} | HTML: {html_file}")
    return out
